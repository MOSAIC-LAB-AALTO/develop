...
//Definition to manage GTP protocol device in userspace
...
//Inserting code blocks for the GTP protocol HEADER
enum gtpu {
	GTPU_TUN_SPECIAL,
	GTPU_TUN_VERSION,
	GTPU_TUN_PROTOCOL_TYPE,
	GTPU_TUN_RESERVED,
	GTPU_TUN_EXTENSION_HEADER_FLAG,
	GTPU_TUN_SEQUENCE_NUMBER_FLAG,
	GTPU_TUN_NPDU_NUMBER_FLAG,
	GTPU_TUN_MESSAGE_TYPE,
	GTPU_TUN_MESSAGE_LENGTH,
	GTPU_TUN_TEID,
	GTPU_TUN_SEQUENCE_NUMBER,
	GTPU_TUN_NPDU_NUMBER,
	GTPU_TUN_NEXT_EXTENSION_HDR_TYPE,
	GTPU_TUN_MAX,
};
...
enum gtpc {
	GTPC_TUN_SPECIAL,
	GTPC_TUN_VERSION,
	GTPC_TUN_PIGGYBACKING_FLAG,
	GTPC_TUN_TEID_FLAG,
	GTPC_TUN_SPARE,
	GTPC_TUN_MESSAGE_TYPE,
	GTPC_TUN_MESSAGE_LENGTH,
	GTPC_TUN_TEID,
	GTPC_TUN_SEQUENCE_NUMBER,
	GTPC_TUN_SPARE_NUMBER,
	GTPC_TUN_MAX,
};
...
static int dump_gtp(NetDeviceEntry *nde, struct cr_imgset *imgset, struct nlattr **info)
{
    GtpEntry ge = GTP_ENTRY__INIT;
    int ret;
    struct nlattr *data[GTPA_MAX];
	uint32_t gtp_ms_addr, gtp_peer_addr, gtpu_version;

    if (!info || !info[IFLA_INFO_DATA]) {
        pr_err("no data for gtp\n");
        return -1;
    }

	pr_info("Some GTPU data available\n");
    ret = nla_parse_nested(data, GTPA_MAX, info[IFLA_INFO_DATA], NULL);
    if (ret < 0) {
        pr_err("failed to parse gtpu data\n");
        return -1;
    }

#define ENCODE_ENTRY(__type, __ifla, __proto)	do {			\
		if (data[__ifla]) {					\
			ge.__proto = *(__type *)nla_data(data[__ifla]);	\
			if (ge.__proto)					\
				ge.has_##__proto = true;		\
		}							\
	} while (0)

	if (data[GTPA_PEER_ADDRESS]) {
		gtp_peer_addr = *(u32 *)nla_data(data[GTPA_PEER_ADDRESS]);
		if (gtp_peer_addr != 0) {
			ge.n_peer_addr = 1;
			ge.peer_addr = &gtp_peer_addr;
		}
	}

	if (data[GTPA_MS_ADDRESS]) {
		gtp_ms_addr = *(u32 *)nla_data(data[GTPA_MS_ADDRESS]);
		if (gtp_ms_addr != 0) {
			ge.n_ms_addr = 1;
			ge.ms_addr = &gtp_ms_addr;
		}
	}

	ENCODE_ENTRY(u32, GTPA_VERSION, version);
	ENCODE_ENTRY(u32, GTPA_LINK, link);
	ENCODE_ENTRY(u16, GTPA_FLOW, flow);
	ENCODE_ENTRY(u32, GTPA_NET_NS_FD, net_ns_fd);

	gtpu_version = *(u32 *)nla_data(data[GTPA_VERSION]);
	switch (gtpu_version){
		case GTP_V0:
			 ...
			 */
			ENCODE_ENTRY(u16, GTPA_FLOW, flow);
			ENCODE_ENTRY(u64, GTPA_TID, tid); //Not sure if this is correctly encoded
			ENCODE_ENTRY(u32, GTPA_PAD, pad); //Not sure if this is needed
			break;
		case GTP_V1:

			ENCODE_ENTRY(u32, GTPA_I_TEI, i_teid);
			ENCODE_ENTRY(u32, GTPA_O_TEI, o_teid);
			break;
		default:
			pr_err("Unknown GTPU version!\n");
			break;
	}
...
/* GTP link netdev is a type ARPHRD_NONE
 * so dump_one_gendev() should work for gtp too
 
static int dump_one_gtp(struct ifinfomsg *ifi, char *kind,
						struct nlattr **tb, struct cr_imgset *fds)
{
	char *name;

	if (strcmp(kind, "gtp")) {
		pr_err("GTP device with %s kind\n", kind);
		return -1;
	}

	name = (char *)RTA_DATA(tb[IFLA_IFNAME]);
	if (!name) {
		pr_err("gtp device %d has no name\n", ifi->ifi_index);
		return -1;
	}
...
static int dump_one_link(struct nlmsghdr *hdr, void *arg)
{
	...
	case ARPHRD_ETHER:
		...
		break;
	case ARPHRD_NONE:
		pr_info("This could be a gtp tunnel hello world!\n");
		ret = dump_one_gendev(ifi, kind, tb, fds);
		//ret = dump_one_gtp(ifi, kind, tb, fds);
		break;
	case ARPHRD_VOID:
...
//GTP: restoring the GTP device link
static int gtp_link_info(NetDeviceEntry *nde, struct newlink_req *req)
{
	struct rtattr *gtp_data;
	GtpEntry *ge = nde->gtp;

	if (!ge){
		pr_err("Missing gtp entry %d\n", nde->ifindex);
		return -1;
	}

	addattr_l(&req->h, sizeof(*req), IFLA_INFO_KIND, "gtp", 3);
	gtp_data = NLMSG_TAIL(&req->h);
	addattr_l(&req->h, sizeof(*req), IFLA_INFO_DATA, NULL, 0);
...
#define DECODE_ENTRY(__type, __ifla, __proto) do {				\
	...
	if (ge->n_peer_addr){
		if (ge->n_peer_addr != 1){
			pr_err("Too long peer addr for gtp\n");
			return -1;
		}
		addattr_l(&req->h, sizeof(*req), GTPA_PEER_ADDRESS, ge->peer_addr, sizeof(u32));
	}
	...
	if (ge->n_ms_addr){
		...
		addattr_l(&req->h, sizeof(*req), GTPA_MS_ADDRESS, ge->ms_addr, sizeof(u32));
	}

	DECODE_ENTRY(u32, GTPA_VERSION, version);
	DECODE_ENTRY(u32, GTPA_LINK, link);
	DECODE_ENTRY(u16, GTPA_FLOW, flow);
	DECODE_ENTRY(u32, GTPA_NET_NS_FD, net_ns_fd);
	...
	switch (ge->version) {
		case GTP_V0:
			 ...
			 */
			DECODE_ENTRY(u16, GTPA_FLOW, flow);
			DECODE_ENTRY(u64, GTPA_TID, tid); //Not sure if this is correctly encoded
			DECODE_ENTRY(u32, GTPA_PAD, pad); //Not sure if this is needed
			break;
		case GTP_V1:

			DECODE_ENTRY(u32, GTPA_I_TEI, i_teid);
			DECODE_ENTRY(u32, GTPA_O_TEI, o_teid);
			break;
		default:
			pr_err("Restoring part unknown GTPU version!\n");
			break;
	}
#undef DECODE_ENTRY

			gtp_data->rta_len = (void *)NLMSG_TAIL(&req->h) - (void *)gtp_data;
...
static int restore_link(NetDeviceEntry *nde, int nlsk, int criu_nlsk)
{
	...
	case ND_TYPE__GTP:
		pr_info("This is probably our gtp tunnel\n");
		/*
		 * let's restore the gtp_link from here.
		 */
		return restore_one_link(nde, nlsk, gtp_link_info, NULL);
